...
public class Level4 extends JPanel implements ActionListener, KeyListener{
	...
	FlyingRectangle fr3 = new FlyingRectangle(140, y2, 140, 20);
	...
	public Level4(int damage2, int health2, String state, String state2, String state3, String state4, String powered) {
		...
		obs.add(new Rectangle(0, 350, 140, 20));
		
		obs.add(new Rectangle(280, 140, 140, 20));
		obs.add(new Rectangle(420, 120, 140, 20));
		obs.add(new Rectangle(560, 100, 140, 20));
		obs.add(new Rectangle(140, 660, 560, 20));
		...
		Thread t = new Thread() 
        {
			...
			public void run()
			{
				...
				{  	
					...
					for (Rectangle r : obs) 
					{
						if (player.intersects(obs.get(4)) && !player.intersects(fr3)) 
						{
							health -= 15;
						}
							...
							if (player.intersects(r) && !isVertical(r)) 
							{
								...
								if (!(r instanceof FlyingRectangle)) {
									 c.playSound(new File("sound/spike.WAV"));
								}
							}
					...
					if (reachedEnd()) {
						...
						Level5 nt = new Level5(damage, health, pass, pass2, pass3, pass4, pass5);
					...
					if (reachedAlternateEnd()) {
						...
						SecretLevel nt = new SecretLevel(damage, health, pass, pass2, pass3, pass4, pass5);
	...
	{   
	  ...
	  // en2.paintComponent(g);
	   ...
	   for (Rectangle o : obs) 
	   {
		   ...
		   g.drawImage(scaleImage(o.width, o.height / 2, "image/pike.png"), o.x, o.y - o.height / 2, null);
	   }
	   ...
	   if (en.getHealth() < 0)
	   {
		  db.paintComponent(g);
		   ...
		   if (cond5) 
		   {
			   c.playSound(new File("sound/dead.WAV"));
	...
	public boolean reachedAlternateEnd() {
		if (x > 700 &&  y > 250) {
			return true;
